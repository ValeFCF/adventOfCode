...
var passportArray = input.components(separatedBy: "\n\n")
passportArray.count
//print(passportArray)

func firstPart() -> [String] {
    var passportsValid = [String]()
    for item in passportArray {
        if item.contains("byr:") && item.contains("iyr:") &&
            item.contains("eyr:") && item.contains("hgt:") &&
            item.contains("hcl:") && item.contains("ecl:") &&
            item.contains("pid:") {
            passportsValid.append(item)
        }
    }
    return passportsValid
}

print("first part = \(firstPart().count)")

func secondPart() -> Int {
    var howManyAreValid = 0
    let array = firstPart()
    for item in array {
        let arraySeparated = item.components(separatedBy: .whitespacesAndNewlines)
        //print(arraySeparated)
        var numValidation = 0
        for keyAndValue in arraySeparated {
            let keyAndValueArray = keyAndValue.components(separatedBy: ":")
            switch keyAndValueArray[0] {
            case "byr":
                numValidation += yearValidation(keyAndValueArray[1], atLeast: 1920, atMost: 2002) ? 1 : 0
            case "iyr":
                numValidation += yearValidation(keyAndValueArray[1], atLeast: 2010, atMost: 2020) ? 1 : 0
            case "eyr":
                numValidation += yearValidation(keyAndValueArray[1], atLeast: 2020, atMost: 2030) ? 1 : 0
            case "hgt":
                numValidation += heightValidation(keyAndValueArray[1]) ? 1 : 0
            case "hcl":
                numValidation += colorValidation(keyAndValueArray[1]) ? 1 : 0
            case "ecl":
                numValidation += eclValidation(keyAndValueArray[1]) ? 1 : 0
            case "pid":
                numValidation += passportIDValidation(keyAndValueArray[1]) ? 1 : 0
            default:
                numValidation += 0
            }
        }
    ...
    return howManyAreValid
}

print("second part = \(secondPart())")

func yearValidation(_ string: String, atLeast: Int, atMost: Int) -> Bool {
    //print(string)
    let number = Int(string) ?? 0
    return number >= atLeast && number <= atMost
}

func heightValidation(_ string: String) -> Bool {
    if string.contains("cm") {
        let value = string.dropLast()
        let secondValue = value.dropLast()
        let number = Int(secondValue) ?? 0
...
func colorValidation(_ string: String) -> Bool {
    guard let _ = string.range(of: "^#[0-9a-f]{6}", options: .regularExpression) else {
        return false
    }
    return true
}

func eclValidation(_ string: String) -> Bool {
    //print(string)
    guard let _ = string.range(of: "^(amb|blu|brn|gry|grn|hzl|oth)",
