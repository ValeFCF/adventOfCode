...
typealias PasswordAndRulesAlias = (String, String, String)

var passwordsAndRules = [PasswordAndRulesAlias]()
for stringElement in stringArray {
    let element = stringElement.split(separator: " ")
    let firstRule: String = String(element[0])
    let secondRule: String = String(element[1])
        .replacingOccurrences(of: ":", with: "")
    let password: String = String(element[2])
    passwordsAndRules.append((firstRule, secondRule, password))
}

//print(passwordsAndRules)

func firstPart() -> Int {
    var timesOfValidPassword = 0
    for item in passwordsAndRules {
        let numbersRule = item.0.split(separator: "-")
        let firstRule: Int = Int(numbersRule[0]) ?? 0
        let secondRule: Int = Int(numbersRule[1]) ?? 0
        
        let tok = item.2.components(separatedBy: item.1)
        let numberOfOcurrences = tok.count-1

        if numberOfOcurrences >= firstRule && numberOfOcurrences <= secondRule {
            timesOfValidPassword += 1
        }
    }
    return timesOfValidPassword
}
...
func secondPart() -> Int {
    ...
    for item in passwordsAndRules {
        let numbersRule = item.0.split(separator: "-")
        let firstNumberRule: Int = Int(numbersRule[0]) ?? 0
        let secondNumberRule: Int = Int(numbersRule[1]) ?? 0
        
        var wordArray = [String]()
        for (_, letter) in item.2.enumerated() {
            wordArray.append(String(letter))
        }
        
        var hasInTheFirst = false
        if wordArray[firstNumberRule-1] == item.1 {
            hasInTheFirst = true
        }
        
        var hasInTheSecond = false
        if wordArray[secondNumberRule-1] == item.1 {
            hasInTheSecond = true
        }
        
        if hasInTheFirst != hasInTheSecond {
            timesOfValidPassword += 1
        }
